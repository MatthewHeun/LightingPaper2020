...
<<Weighting_Function_Graph_Exergy, cache=use_cache, cache.extra=openssl::md5(chroma_type), fig.width=1.5, fig.height=5>>=
response_data %>%
...
<<Responses_Graph_Exergy, cache=use_cache,cache.extra=openssl::md5(chroma_type), fig.width=4.5, fig.height=5>>=
response_data %>%
  ggplot(mapping = aes(x = `Wavelength [nm]`)) +
