...
<<Vis_Graph, cache=use_cache, fig.align="center", fig.width = 5, fig.height = 3, fig.cap="Normalized sensitivity and color perception of the human eye to EM radiation~\\cite{schubert2006}, according to the photopic luminous weighting function~($V_\\lambda$).">>=
response_data %>% 
...
Furthermore, the useful energy in light must undergo a `quality adjustment` to exergy,
as radiation contains not only energy, but entropy, 
which renders a portion of the energy in light unavailable for work. 
In this regard \citeauthor{Delgado-Bonal:2017aa} has importantly demonstrated that 
the human eye's peak sensitivity is more closely correlated 
with a quality-adjusted measure of light, as opposed to simply energy.
This emphasizes the importance of moving from energy to exergy.
...
of that stream of monochromatic photons,
in accordance with~\citeauthor{Shan2019}.
...
print(xtable::xtable(results_etas_fin, digits=2, 
                     ...
                     the unweighted energy efficiency~($\\eta_{E,uw}$) is  
                     underestimated;
...
print(xtable::xtable(results_etasX_fin, digits=2, 
                     ...
                     underestimated for the incandescent (INC) and halogen (HAL) lamps;
                     the SPD data do not go beyond 899~nm, 
                     despite INC and HAL emission far into 
                     the infrared portion of the EM spectrum.", 
...
\begin{enumerate}[label={(\alph*)}]
  ...
  \item the maximum luminous efficacy ($K_{max}$) is taken to be 683~lm/W, 
...
some authors have mistakenly, we now know, taken $K_{max}$ to be 
...
\begin{enumerate}[label={(\alph*)}]
  ...
  \item it allows choices for the spectral luminous weighting function~($f_\lambda$), 
        ...
        thereby enabling our second recommendation, namely use the universal luminous weighting function ($U_\lambda$).
...
we recommend that an approximate method can be used, as discussed in the next section.
...
for development of the approximate method.
...
$\bar{\phi}_{L,pl} = \Sexpr{format(round(phi_mean, 3), nsmall = 3)}$ 
...
Average photopic-to-universal conversion factors~($\gammaratavg{}$) by lamp technology
...
ideally in the form of the fraction of lighting electricity consumed by each lamp type.
