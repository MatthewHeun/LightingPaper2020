...
\begin{equation} \label{eq:approximate_unknown_tech}
  \eta_{X,univ} \approx (\Sexpr{format(round(phi_mean, 2), nsmall = 2)}) (\Sexpr{format(round(conversionX_mean, 2), nsmall = 2)}) \frac{K}{683 \text{ lm/W}} = \Sexpr{format(round(phi_and_gammaX_factor, 2), nsmall = 2)} \frac{K}{683 \text{ lm/W}} \; .
\end{equation}
%
If the lamp technology is known, 
$\bar{\phi}_{L,pl}$ and $\gammaratavg{}$ values from Tables~\ref{tab:mean_phi_p2_by_lamp_type} and~\ref{tab:conversionfactors} should be used instead of the overall averages.
