...
(\humaneyesensitivity{}).
Thus, light is EM radiation within that narrow wavelength range~\cite{campbell2017}, 
...
Practitioners
**** 
PB: is there a better word than ``practitioners''?
Maybe ``Analysts''?---MKH
****
who use luminous efficacy as a lamp efficiency metric include \citet{Nordhaus1996} 
...
Conventionally, societal exergy analysts solve this problem by estimating
...
The conventional method is typically implemented 
...
namely what value should be used for the theoretical maximum luminous efficacy~($K_{max}$)?
...
a ***** PB: find the reason for 200~lm/W.---MKH ****, broad-spectrum white light, and green light at 555~nm, respectively.
Practically, \citeauthor{Paoli2020} calculated that the upper limit for the luminous efficacy of an LED is 284 - 350 lm/W,
when considering all stages of energy loss.
As can be seen from Figure~\ref{fig:LuminousEfficacy_Graph} the exponential improvement 
in LED luminous efficacies suggest this limit is within reach,
as predicted by Haitz's law \cite{haitz1999case, graydon2007haitz}, 
which stated \emph{``Every decade, for a given wavelength of light, 
the cost per lumen falls by a factor of 10 
and the amount of light generated per LED package increases by a factor of 20.''}.

**** Is this the best place to include Haitz's law. Or should we still include it? 
It was mentioned in the meeting the other day that the focus of the paper is on single lamps, 
not efficacy/efficiency trends over time. ---ZM ****
...
the exergy of light and the exergetic efficiency of lamps, 
further issues and confusions await. 
...
requires that energy is conserved ($\dot{Q}_1 = \dot{Q}_2 + \dot{W}_3$).
...
human skin is differentially responsive to EM spectrum wavelength~\citep{Anderson:1981aa}.  
...
Similarly, human eyes perceive only a (narrower) portion of the EM radiation spectrum
...
we need both luminous weighting functions (Section~\ref{sec:weighting_functions}) and
example lamps.
...
spectral luminous weighting functions~($f_\lambda$),
...
\begin{enumerate}[label={(\alph*)}]
  ...
  \item the spectral luminous weighting function~($f_\lambda$) is taken to be
...
\end{equation}
%
Specifically, the conventional method's estimate 
...
the analyst is required to select a value for maximum luminous efficacy ($K_{max}$), 
...
some authors have assumed $K_{max}$ to be 
...
\begin{enumerate}[label={(\alph*)}]
  ...
  \item it allows choices for the spectral luminous weighting function~($f_\lambda$), 
...
requires the use of the photopic luminous weighting function and cannot do any different.
...
Our third recommendation is for an approximate method to determining 
...
but it provides energy efficiency (instead of exergetic efficiency)
while requiring the use of photopic luminosity function~($\eta_{E,L,v,pl}$).
In contrast, we recommend that,
in the absence of SPD data,
societal exergy analysts
calculate exergetic efficiency 
with the universal luminous weighting function~($\eta_{X,L,v,univ}$).

We suggest the following two-step process:
%
\begin{enumerate*}[label={(\alph*)}]
	
  \item move from valuable energy efficiency~($\eta_{E,L,v,pl}$)
        to valuable exergetic efficiency ($\eta_{X,L,v,pl}$)
        using an average exergy-to-energy ratio~($\bar{\phi}_{L,pl}$) and 
  
  \item move from valuable exergetic efficiency determined with the photopic luminous weighting function~($\eta_{X,L,v,pl}$)
        to valuable exergetic efficiency determined with the universal luminous weighting function~($\eta_{X,L,v,univ}$)
        using an average ratio between those efficiencies ($\gammaratavg{}$).
    
\end{enumerate*}

**** ZM: 
Delete this paragraph.---MKH
**** 
...
****
MKH:
I would be very reticent to call the data we have used `our database' 
as it is all from the LSPDD. I have rewritten this paragraph below. Let me know if you agree.---ZM
****

****
ZM: 
Thanks for this point. I agree.
You can delete the outdated paragraph above and these comments.---MKH
****
...
(The sample consists of 24 lamp SPDs representing six technologies: 
INC, Halogen (HAL), HPS, Metal halide (MH), CFL, and LED)
The average value is $\bar{\phi}_{L,pl} = 0.958$ **** ZM: Make sure 0.958 is the correct value.---MKH **** with a
remarkably small standard deviation of $\sigma_{\phi_{L,pl}} = ****$.
...
****
MKH:
Before I do this, at the moment we are using the mean phi value across all lamps and weighting functions,
do we want to change this to the mean value for the universal luminosity function,
or photopic luminosity function? --- ZM
****

****
ZM:
Good question. 
I think we want to use the mean $\phi_{L,pl}$ function for the photopic luminosity function~($V_\lambda$), 
because our proposed approximate method is to FIRST move from energy to exergy under the photopic luminosity function~($V_\lambda$)
THEN move from photopic to universal in exergy space.
You'll note that I changed from $\phi_L$ to $\phi_{L,pl}$ to be more explicit about 
the issue of which luminous weighting function should be used.---MKH
****

****
ZM:
Can you include a table of $\phi_{L,pl}$ by technology, too, similar to Table~\ref{tab:conversionfactors}?
Should it contain all 6 technologies?---MKH
****
...
compared to the photopic luminosity weighting function~($V_\lambda$), 
...
with a small standard deviation of $\sigma_{\gammarat{}} = 0.108$.

Average conversion factors~($\gammaratavg{}$) by lamp technology are given in Table~\ref{tab:conversionfactors}.
...
print(xtable::xtable(conversion_table, digits=2, 
                     caption = "The mean and standard deviation across all sample lamps
                     for factors ($\\gammaratavg{}$) for converting data for the final-to-useful
                     energy efficiency of a lamp based on the photopic luminosity function,
                     to final-to useful energy efficiencies based on the universal luminosity function.
                     **** ZM: Can you change the sample size row to be integers?
                     Also, I would put the sample size on the last row. 
                     Top row: mean.
                     Second row: stdev.
                     Third row: sample size.
                     Finally, should this table contain all *6* technologies instead of the 
                     technologies of the four example lamps?
                     I think it should.---MKH ****",
...
\begin{equation} \label{eq:approximate}
  \eta_{X,L,v} \approx \bar{\phi}_{L,pl} \, \gammaratavg{} \, \frac{K}{683 \text{ lm/W}} \; ,
\end{equation}
%
which is a modification of Equation~\ref{eq:K_over_e_max}
that accounts for the move from energy to exergy (via $\bar{\phi}_{L,pl}$)
...
$\bar{\phi}_{L,pl}$ and $\gammaratavg{}$ values from **** Tables~X and~Y **** should be used instead of the overall averages.
...
Two types of data are required for a given period of interest, typically a year:
%
\begin{enumerate*}[label={(\alph*)}]

  \item valuable exergetic efficiency data for each lamp technology~($\eta_{X,L,v,i}$) and
...
but depends on the presence of each lamp's SPD,
...
luminous efficacy data must be converted into valuable exergetic efficiency values 
using conversion factors, namely $\bar{\phi}_{L,pl}$ and $\gammaratavg{}$, in the approximate method.
(See Section~\ref{sec:approximate_method}.)

Importantly, data for the usage proportion for each lamp technology~($\theta_i$)
for each year of the study are needed,
ideally in the form of the proportion of lighting electricity consumed by each lamp type.
Such data (and luminous efficacy values)
are provided by the US DOE's Lighting Market Characterization reports \citep{DOE2002, DOE2012, DOE2017}
...
****
MKH:
The above equation is correct, but i believe mine was also.
The difference being that mine expressed the exergy efficiency
as a product of the energy efficiency and the exergy-to-energy ratio,
which i believe we should do, so it is the same format as the equation below---ZM 
****

****
ZM:
Thanks for the clarification.
I think I want to use ``my'' version, because I'm writing the text above 
as though the analyst is using the exact method and thus 
obtains $\eta_{X,L,v,i}$ directly from the exact method.
The paragraph below talks about what to do when the exact method won't work.
Does that seem OK to you?---MKH
****
...
\begin{equation} \label{eq:agg_eta_K}
  \eta_{X,L,v,agg} \approx \sum \limits_{i} \theta_{i} \, \bar{\phi}_{L,pl} \, \gammaratavg{} \, 
                            \frac{K_{i}}{683 \text{ lm/W}} \; .
\end{equation}

****
MKH:
Here as we have specified that we are converting efficiency values based on luminous efficacy values
to an efficiency based on the universal luminosity function using $\gammaratavg{}_{,i}$
the subscript v in $\eta_{X,L,v,agg}$ should become univ, 
as the valuable portion of light is now determined by the universal luminosity function.
This reduces the number of subscripts!
Also as the mean conversion factor and mean exergy-to-energy ratios we have used are averaged across lamps they should not have the subscript i. 
I have made these changes, let me know if you agree/disagree!----ZM
****

****
ZM: 
I agree about removing the subscript $i$.
I haven't made the $v \rightarrow univ$ change, because I think that requires a broader discussion.
We would need to make that change in very many places, not just here.
I'm willing to have that discussion.
But I don't think I should make a change here only.---MKH
****
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item uses the correct spectral exergy-to-energy ratio~($\phi_{L,\lambda}$), and
...
we developed a novel approximate method,
involving surprisingly stable values across lighting technologies 
of the exergy-to-energy ratio~($\phi_{L,\lambda}$) and 
the photopic-to-universal factor~($\gammarat{}$).
