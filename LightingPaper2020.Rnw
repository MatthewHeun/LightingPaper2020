...
with a remarkably small standard deviation of $\sigma_{\phi_{L,pl}} = \Sexpr{format(round(phi_sd, 2), nsmall = 2)}$.
...
$\bar{\phi}_{L,pl}$ and $\gammaratavg{}$ values from Tables~\ref{mean_phi_p2_by_lamp_type} and~\ref{conversionfactors} should be used instead of the overall averages.
