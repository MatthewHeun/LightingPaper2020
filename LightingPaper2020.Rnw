...
(b)~use the universal luminous weighting weighting function, and
...
Practically, \citeauthor{Paoli2020} estimated 
the upper limit for the luminous efficacy of an LED to be 284--350~lm/W,
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item use the universal luminous weighting function~($U_\lambda$), and
...
Adopting the universal luminous weighting function~($U_\lambda$) will encourage manufactures to include 
...
(The sample consists of 24
****
ZM: Replace ``24'' with Sexpr---MKH
****
lamp SPDs representing six technologies: 
INC, Halogen (HAL), HPS, Metal halide (MH), CFL, and LED)
The average value is $\bar{\phi}_{L,pl} = 0.958$ 
**** 
ZM: 
Replace ``0.958'' with Sexpr.---MKH 
**** 
with a
...
do we want to change this to the mean value for the universal luminous weighting function,
or photopic luminous weighting function? --- ZM
****

****
ZM:
Good question. 
I think we want to use the mean $\phi_{L,pl}$ function for the photopic luminous weighting function~($V_\lambda$), 
because our proposed approximate method is to FIRST move from energy to exergy under the photopic luminous weighting
function~($V_\lambda$)
...
****
ZM:
Replace ``1.37'' and ``0.108'' with Sexpr.---MKH
****
...
****
ZM:
Replace ``0.958'' and ``1.37'' and ``1.31'' with Sexpr.---MKH
****
