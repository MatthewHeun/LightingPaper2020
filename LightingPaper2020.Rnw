...
conversion_count <- overall_stats_X$count

conversion_mean <- overall_stats_X$mean_conv
  
conversion_sd <- overall_stats_X$sd_conv
@
...
phi_and_gamma_factor <- conversion_mean * phi_mean
@
...
was $\gammaratavg{} = \Sexpr{format(round(conversion_mean, 2), nsmall = 2)}$,
with a small standard deviation of $\sigma_{\gammarat{}} = \Sexpr{format(round(conversion_sd, 2), nsmall = 2)}$.
...
\begin{equation} \label{eq:approximate_unknown_tech}
  \eta_{X,univ} \approx (\Sexpr{format(round(phi_mean, 2), nsmall = 2)}) (\Sexpr{format(round(conversion_mean, 2), nsmall = 2)}) \frac{K}{683 \text{ lm/W}} = \Sexpr{format(round(phi_and_gamma_factor, 2), nsmall = 2)} \frac{K}{683 \text{ lm/W}} \; .
\end{equation}
%
