...
You can delete the outdated paragarph above and these comments.---MKH
****
...
(This sample consists of 24 lamp SPDs representing six technologies: 
INC, Halogen (HAL), HPS, Metal halide (MH), CFL, and LED)
The average value of $\phi_{L,pl}$ is $\bar{\phi}_{L,pl} = 0.958$ **** ZM: Make sure 0.958 is the correct value.---MKH **** with a
...
ZM:
Good question. 
I think we want to use the mean $\phi_L$ function for the photopic luminosity function~($V_\lambda$), 
...
print(xtable::xtable(conversion_table, digits=2, 
                     ...
                     **** ZM: Can you change the sample size row to be integers?
                     ...
                     technologies of the four example lamps?---MKH ****",
...
$\bar{\phi}_L$ and $\gammaratavg{}$ values from **** Tables~X and~Y **** should be used instead of the overall averages.
...
the values for $\bar{\phi}_L$ and $\gammaratavg{}$ shown
...
When SPDs are available, the aggregate valuable exergetic lighting efficiency for a particular year can be found by
%
...
\begin{equation} \label{eq:agg_eta_K}
  \eta_{X,L,univ,agg} \approx \sum \limits_{i} \theta_{i} \, \bar{\phi}_{L} \, \gammaratavg{} \, 
