...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item move from valuable energy efficiency~($\eta_{E,L,v,pl}$)
        ...
        using an average exergy-to-energy ratio~($\bar{\phi}_L$) and 
...
****
MKH:
I would be very reticent to call the data we have used 'our database' as it is all from the LSPDD. I have rewritten this paragraph below. Let me know if you agree ---ZM
****

We calculated the exergy-to-energy ratio for a sample of lamps from the LSPDD \cite{aube2013}
using Equation~\ref{eq:phi_L}. 
(This sample of 24 lamps with SPDs representing six technologies: 
...
print(xtable::xtable(results_cea_table, digits=2, 
                     ...
                     Note that values for the conventional method are the same as those 
                     in row 3 of Table~\\ref{tab:results_etas};
                     and values for the exact method are the same as those
                     in row 4 of Table~\\ref{tab:results_etasX}.", 
