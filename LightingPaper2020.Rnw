...
\ins{overestimation of exergetic efficiency by a factor as large as 3.}
In response, 
...
\rev{Thus}{Therefore}, the efficiency of energy and exergy conversion of electricity
...
``what is the maximum work we could obtain from a stream of \rev{this}{the} physical property?''
...
\end{equation}
%
\del{because the second law of thermodynamics 
states that exergy is always destroyed ($\dot{X}_1 > \dot{X}_2 + \dot{X}_3$ or 
$\dot{X}_D = \dot{X}_1 - (\dot{X}_2 + \dot{X}_3)$).}
\ins{because all real processes are internally irreversible, such that 
...
\end{equation}
%
\ins{(See~\citet{benenson2006handbook}.)}
...
Note that exergy-to-\rev{exergy}{energy} ratio for light~($\phi_{L,\lambda}$) is
