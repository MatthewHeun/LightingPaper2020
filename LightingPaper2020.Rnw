...
\ins{(as shown in Equation~\ref{eq:phi_L_lambda_shan_zhou})}, but the conventional method assumes it is.
...
That said, we suggest that the choice of weighting function\del{ ($f_\lambda$)} in societal exergy analysis
