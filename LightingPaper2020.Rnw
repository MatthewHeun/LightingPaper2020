...
(b)~use the universal luminous weighting function, and
(c)~use the reasonable approximation to the exact method 
    when luminous efficacy is known
    but the spectral power distribution is unavailable.
}
...
<<Vis_Graph, cache=use_cache, fig.align="center", fig.width = 5, fig.height = 3, fig.cap="Normalized sensitivity of the human eye to EM radiation~\\cite{schubert2006}, according to the photopic luminous weighting function~($V_\\lambda$).">>=
response_data %>% 
...
to light by electric lamps is the focus of this article.
...
Instead, light is quantified in lumens.
...
<<LuminousEfficacy_Graph, cache=use_cache, fig.align="center", fig.width = 5, fig.height = 4, fig.cap = "The luminous efficacy of selected lamp technologies from 1880 to 2020. Data were obtained from three sources: the Museum of Electric Lamp Technology (LampTech)~\\cite{hooker_2020}, the ENERGYSTAR database of certified lamps~\\cite{energystar_2020}, and the lighting market characterization reports commissioned by the US Department of Energy (DOE)~\\cite{DOE2002, DOE2012, DOE2017}." >>=
...
a ***** PB: find the reason for 220~lm/W.---MKH ****, 
broad-spectrum white light, and
green light at 555~nm, respectively.

\citeauthor{Paoli2020} estimated 
the practical upper limit for the luminous efficacy of an LED lamp to be 284--350~lm/W,
when considering all stages of energy loss~\citep{Paoli2020}.
Figure~\ref{fig:LuminousEfficacy_Graph} shows this limit is within reach
due to the exponential improvement in luminous efficacies for LED technology.
...
%++++++++++++++++++++++++++++++
\subsection{Aim, contributions, and structure}
\label{sec:aim_contribution_structure}
%++++++++++++++++++++++++++++++

The need discussed above leads to the \emph{aim} of this article: 
to bring clarity to 
%
...
%------------------------------
\subsubsection{\Enaex{} of EM radiation ($\dot{E}_{EM}$ and $\dot{X}_{EM}$)} 
...
See Section~\ref{sec:weighting_functions}.)
...
%------------------------------
\subsubsection{\Enaex{} of light ($\dot{E}_{L}$ and $\dot{X}_{L}$)} 
...
yielding a corrected SPD~($\dot{E}_{EM,\lambda}$) for each lamp 
...
(E.g., the valuable~($v$) energy~($E$) efficiency~($\eta$) of light~($L$) 
...
print(xtable::xtable(results_etas_fin, digits=2, 
                     ...
                     Note that for the incandescent (INC) and halogen (HAL)
                     lamps, 
                     the unweighted energy efficiency~($\\eta_{E,uw}$) is an 
                     underestimate;
                     the SPD data do not go beyond 899~nm, 
                     despite INC and HAL emission far into 
...
is that it incorrectly estimates the 
...
<<ErrorCalcs>>=
correct_eff <- 683 # lm/W
wrong_eff_220 <- 220 # lm/W
wrong_eff_400 <- 400 # lm/W
wrong_rat_220 <- correct_eff / wrong_eff_220
wrong_rat_400 <- correct_eff / wrong_eff_400
@
...
some authors have incorrectly, we now know, taken $K_{max}$ to be 
220~lm/W or 400~lm/W,
thereby overestimating exergetic efficiency of lighting~($\eta_{X,L,v}$)
with the conventional method
by a factor of
$\frac{\Sexpr{correct_eff} \text{ lm/W}}{\Sexpr{wrong_eff_220} \text{ lm/W}} 
  = \Sexpr{format(round(wrong_rat_220, 1), nsmall = 1)}$
or
$\frac{\Sexpr{correct_eff} \text{ lm/W}}{\Sexpr{wrong_eff_400} \text{ lm/W}} 
  = \Sexpr{format(round(wrong_rat_400, 1), nsmall = 1)}$,
respectively.
...
the analyst is required to use the photopic luminous weighting function~($V_\lambda$)
...
with the universal luminous weighting function~($U_\lambda$).
...
provided in the \texttt{R} package \texttt{ggspectra}~\cite{Aphalo2015}.)
...
The photopic luminous weighting function~($V_\lambda$) does not
...
we believe that the universal luminous weighting function ($U_\lambda$)
is more appropriate.
...
Adopting the universal luminous weighting function~($U_\lambda$) will encourage manufacturers to include 
...
Our third recommendation is for an approximate method to determine 
...
<<ConversionX_Calcs, cache=use_cache, results="asis", cache.extra.2=tools::md5sum('Data/univ_conversion.R')>>=
source("Data/univ_conversion.R")

conversion_count <- overall_stats$count

conversion_mean <- overall_stats$mean_conv
  
conversion_sd <- overall_stats$sd_conv
@

<<Conversion_Calcs, cache=use_cache, results="asis", cache.extra.2=tools::md5sum('Data/univX_conversion.R')>>=
source("Data/univX_conversion.R")

conversionX_count <- overall_stats_X$count

conversionX_mean <- overall_stats_X$mean_conv
  
conversionX_sd <- overall_stats_X$sd_conv
@

<<phi_Calcs, cache=use_cache, results="asis", cache.extra.2=tools::md5sum('Data/phi_conversion.R')>>=
source("Data/phi_conversion.R")

phi_conversion <- overall_phi$phi_count

phi_mean <- overall_phi$phi_mean
  
phi_sd <- overall_phi$phi_sd

phi_and_gammaX_factor <- conversionX_mean * phi_mean
@
...
(The sample consists of \Sexpr{conversion_count} lamp SPDs representing six technologies: 
INC, Halogen (HAL), HPS, Metal halide (MH), CFL, and LED)
The average value is $\bar{\phi}_{L,pl} = \Sexpr{format(round(phi_mean, 2), nsmall = 2)}$ 
with a remarkably small standard deviation of $\sigma_{\phi_{L,pl}} = \Sexpr{format(round(phi_sd, 5), nsmall = 5)}$.
...
was $\gammaratavg{} = \Sexpr{format(round(conversionX_mean, 2), nsmall = 2)}$,
with a small standard deviation of $\sigma_{\gammarat{}} = \Sexpr{format(round(conversionX_sd, 2), nsmall = 2)}$.

**** MKH: I believe we need to add a sentence on the approximate equivalency 
of the photopic-to-universal factor in both the energy and exergy spaces.
See paragraph below.---ZM ****
...
(1) If we discuss the photopic-to-universal factor in energy space, 
we will also need to discuss the exergy-to-energy ratio in universal space.
Doing so will add additional complexity to a paper that is quite tidy at present.

(2) Thus, I suggest that we do \emph{not} discuss the photopic-to-universal factor in energy space,
but I'm open to hearing other points of view.

(3) If the rest of you think differently to me, 
I suggest that discussion of the exergy-to-energy ratio in universal space
be pushed to an appendix.
We can always add an appendix to discuss this matter if reviewers ask about it. 

(4) If the rest of you think differently to me, 
we need to be very careful about how we report uncertainty and standard deviation. 
Typically, $\pm$ indicates an uncertainty interval with normal distribution.
The uncertainty interval is different from standard deviation,
which you have reported here.  
($1 \sigma$ is only about 65\% confidence, typically unacceptably low.)

If you want the uncertainty interval at 95\% confidence (typical) and 
have an infinite number of samples and
have a 2-tailed distribution of uncertainty, 
you need $1.96 \sigma$.
(See \url{https://www.sjsu.edu/faculty/gerstman/StatPrimer/t-table.pdf} for the student's t distribution table.)
Given that we have only 5 samples for our technologies (at present), 
we would need $2.571 \sigma$ to obtain a 95\% confidence interval.

I don't think we want to push into this complexity.
Thus, I suggest that (if we add this discussion to an appendix)
we stay away from the confidence interval notation ($\pm$), 
instead favoring reporting simply the mean and standard deviation values for
the photopic-to-universal factor in both the energy and exergy space.---MKH
****

****
MKH:

Thankyou for your response. I now agree. 
I believe we should keep things simple in the main text,
and not include discussion of $\sigma_{\gammarat{}}$ in the energy space.
I'm open to mentioning $\sigma_{\gammarat{}}$ in the appendix however, 
but i'll leave that decision up to you, 
let me know if you'd like me to create the tables for his information.

Thankyou also for clarifying the use of the sd,
I need to go over my statistics, far too rusty!
I also agree, for the sake of simplicity we should simply report the mean and sd.

Please delete my comments and text as required!

---ZM
****


The photopic-to-universal factor ($\gammaratavg{}$) operates in the exergy space 
after application of the exergy-to-energy factor ($\bar{\phi}_{L,pl}$).
However, given that the values of $\gammaratavg{}_{,X}$ and $\gammaratavg{}_{,E}$
are approximately equal, 
$\Sexpr{format(round(conversionX_mean, 3), nsmall = 3)}$
$\pm$ $\Sexpr{format(round(conversionX_sd, 3), nsmall = 3)}$
, and $\Sexpr{format(round(conversion_mean, 3), nsmall = 3)}$
$\pm$ $\Sexpr{format(round(conversion_sd, 3), nsmall = 3)}$
respectively, 
the analyst may use the photopic-to-universal factors ($\gammaratavg{}$) supplied
before or after the application of the exergy-to-energy factor.

**** MKH: Alternatively we provide factors for both the energy and exergy space in the appendix
...
\begin{equation} \label{eq:approximate_unknown_tech}
  \eta_{X,univ} \approx (\Sexpr{format(round(phi_mean, 3), nsmall = 3)}) 
                        (\Sexpr{format(round(conversionX_mean, 2), nsmall = 2)}) 
                        \frac{K}{683 \text{ lm/W}} 
                      = \Sexpr{format(round(phi_and_gammaX_factor, 2), nsmall = 2)} 
                        \frac{K}{683 \text{ lm/W}} \; .
\end{equation}
%
...
could be used instead of the overall averages.
...
\end{enumerate*}
%
(a)--(c) are improvements over the conventional method.
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item use the universal luminous weighting weighting function~($U_\lambda$), and
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item applying the exact method to understand the evolution
        ...
        reducing lighting energy consumption~\citep{CullenPhD}
        (via gains in 
...
a blank excel workbook template in the required format for the insertion of spectral power distribution data by the user. }
...
Emmanuel Aramendia's time was funded by ******.
...
Andrew Stockman and other members of the Colour and Vision Research Laboratory for 
  data for the photopic luminous weighting function~\cite{CVRL2008},
