...
# knitr::knit2pdf("LightingPaper2020.Rnw")
...
(a)~use the exact method when a lamp's spectral power distribution is available, 
(b)~use the universal luminosity weighting function, and
...
In fact, most lamps are rated and advertised by their lumen output and luminous efficacy~($K$).
Figure~\ref{fig:LuminousEfficacy_Graph} shows the evolution over time 
of luminous efficacy for four lamp technologies.
Examples of practitioners using luminous efficacy as a lamp efficiency metric include \citet{Nordhaus1996} 
...
was defined by the Commission Internationale de l' \'{E}clairage (CIE) in 1931
...
emitting exclusively green light at $\lambda = 555 \text{ nm}$.
(See Figure~\ref{fig:Vis_Graph}.) 
...
%++++++++++++++++++++++++++++++
\subsection{Comparison with conventional approach 
...
the exergy-to-energy ratio for light~($\phi_{L,\lambda}$) is not 1, but the conventional method assumes it is.
Condition~(c) highlights an unfortunate requirement for the conventional method, 
...
analysts must reject the conventional method assumption that $\phi_{L,\lambda} = 1$.
...
for the spectral weighting function~($f_\lambda$).
...
We provide three recommendations for societal exergy analysts:
%
\begin{enumerate*}[label={(\alph*)}]
	
  \item use the exact method of Section~\ref{sec:methods} when possible, 
        i.e., when SPD data are available for a lamp, 
  
  \item use the universal luminosity weighting function~($U_\lambda$), and
...
for determining the valuable exergy efficiency of lamps~($\eta_{X,L,v}$)
when SPD data are available for lamps.
...
\begin{enumerate}[label={(\alph*)}]
  ...
  \item it allows choices for the spectral weighting function~($f_\lambda$), 
        ...
        thereby enabling our second recommendation ($f_\lambda = U_\lambda$).

\end{enumerate}


%------------------------------
\subsubsection{Recommendation for the universal weighting function} 
\label{sec:reccuniv}
%------------------------------

Our second recommendation is for use of
the universal luminosity weighting function~($U_\lambda$) in societal exergy analysis
when the purpose of lamps is illumination.

We recognize that the choice of spectral weighting function 
remains contentious~\cite{Delgado-Bonal:2017aa, Pattison2018LEDphotons, Rea2018, Royer2016}.
But we suggest that the choice of weighting function ($f_\lambda$) in societal exergy analysis
...
(Figure~\ref{fig:wfs_and_lamps}  
utilizes the CIE 10 degree color matching function 
provided in the \texttt{R} pacakge \texttt{ggspectra}~\cite{Aphalo2015}.)
This decline in human eye sensitivity toward the infra-red region
is captured well by the photopic luminosity function~($V_\lambda$).
...
The photopic luminosity function~($V_\lambda$) does not
...
we believe that the universal luminosity function ($U_\lambda$)
is more appropriate.
(See also \citet{Rea2018}.)
We provide three arguments in support of this proposal.
...
(For example, see the blue spike in the SPD~($\dot{E}_{EM,\lambda}$)
...
toward the universal luminosity function~($U_\lambda$) 
is possible with the exact method described in Section~\ref{sec:framework},
so long as SPD data are available.
In the absence of SPD data, 
we suggest that an approximate method can be used, as discussed in the next section.
...
Our third recommendation is for an approximate method for determining 
...
In contrast, we want to calculate exergy efficiency 
with the universal luminosity function~($\eta_{X,L,v,univ}$)
in the absence of SPD data.
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item move from valuable energy to valuable exergetic efficiency 
        ($\eta_{E,L,v,pl} \rightarrow \eta_{X,L,v,pl}$)
        using an average exergy-to-energy ratio~($\phi_L$) and 
  
  \item move from the photopic luminosity function
        to the universal luminosity function
        ($\eta_{X,L,v,pl} \rightarrow \eta_{X,L,v,univ}$) using 
        an average pl$\rightarrow$univ factor~($\gammarat{}$).
    
\end{enumerate*}

We calculated the exergy-to-energy ratio for each lamp in our database using
Equation~\ref{eq:phi_L}. 
(The database consists of **** lamps with SPDs representing **** technologies, 
**** list the technologies here ****. )
The average value of $\phi_L$ is $\bar{\phi}_L = 0.958$ with a
remarkably small standard deviation of $\sigma_{\phi_L} = ****$.

Similarly, we obtained an approximate value for $\gammarat{}$ by taking the quotient of 
the valuable exergetic efficiency of each lamp in our database
using the universal luminosity function ($\eta_{X,L,v,univ,i}$)
and the photopic luminosity function ($\eta_{X,L,v,pl,i}$):
%
\begin{equation} \label{eq:conversionuniversal}
  \gammarat{}_{,i} = \frac{\eta_{X,L,v,univ,i}}{\eta_{X,L,v,pl,i}} \; .
\end{equation}
%
Because a greater proportion of each SPD is considered to be light
by the universal luminosity weighting function~($U_\lambda$)
as opposed to the photopic luminosity weighting function~($V_\lambda$), 
$\gammarat{}$ is expected to be greater than 1.
The average value of $\gammarat{}$ across our population of lamps 
was $\gammaratavg{} = 1.37$,
with a small standard deviation of $\sigma_{\gammarat{}} = ****$.
****
ZM: 
Please fill the standard deviation values above.---MKH
****
Average conversion factors~($\gammaratavg{}$) by lamp technology are given in Table~X.
...
\begin{equation} \label{eq:approximate}
  \eta_{X,L,v} \approx \bar{\phi}_{L} \, \gammaratavg{} \, \frac{K}{683 \text{ lm/W}} \; ,
\end{equation}
%
which is a modification of Equation~\ref{eq:K_over_e_max}
that accounts for the move from energy to exergy (via $\bar{\phi}_L$)
and the move from the photopic luminosity function to the universal luminosity function
(via $\gammaratavg{}$).

When the lamp technology is unknown, overall average values can be used
for $\bar{\phi}_{L}$ and $\gammaratavg{}$:
%
\begin{equation} \label{eq:approximate_unknown_tech}
  \eta_{X,L,v} \approx (0.958) (1.37) \frac{K}{683 \text{ lm/W}} = 1.31 \frac{K}{683 \text{ lm/W}} \; .
\end{equation}
%
If the lamp technology is known, 
$\bar{\phi}_L$ and $\gammaratavg{}$ values from Table~X should be used instead of the overall averages.
****
ZM:
Please complete Table~X---MKH
****

Table~\ref{tab:threemethods} summarizes 
the valuable exergetic efficiency 
for the four representative lamps (INC, HPS, CFL, LED) and
the three methods (conventional, exact, and approximate).
The ``Approximate method'' row was generated using
the factors in Equation~\ref{eq:approximate_unknown_tech}.

**** 
ZM:
Something is wrong with Table~\ref{tab:threemethods}.
The conventional row should be the same as row 3 of Table~\ref{tab:results_etas}.
the exact row should be the same as row 4 of Table~\ref{tab:results_etasX}.
Can you check to see if something is going wrong?---MKH
****

**** 
ZM: 
Also, can you change the row names to include the correct variable names?
For example, the first row should be titled
``Conventional method ($\eta_{E,L,V,pl}$, same as row 3 of Table~\ref{tab:results_etas})''
The second row should be titled 
``Exact method ($\eta_{X,L,V,univ}$, same as row 4 of Table~\ref{tab:results_etasX})''.
I don't think we have a name for the bottom row.
Note that the second word should not be capitalized.---MKH
****
...
so analysts must weight per-lamp-technology efficiencies
by each technology's usage fraction.
Several types of data are required for a given period of interest, typically a year:

\begin{enumerate}[label={(\alph*)}]

  \item valuable exergetic efficiency data for each lamp technology~($\eta_{X,L,v,i}$),
  
  \item corresponding exergy-to-energy ratios for each lamp technology~($\phi_{L,i}$), and
...
using conversion factors via the approximate method, namely $\phi_{L}$ and $\gammarat{}$,
described above in Section~\ref{sec:approximate_method}.

Importantly, data for the proportion of each lamp technology 
in use for each year of the study is needed,
ideally in the form of the economy-wide electricity consumption by lamp technology.
For example, the US DOE's Lighting Market Characterization reports provide such data for the USA
in 2001~\cite{DOE2002}, 2010~\cite{DOE2012}, and 2015~\cite{DOE2017}, 
along with associated luminous efficacy values
for the following lamp technologies: incandescent, halogen, compact fluorescent,
linear fluorescent, high intensity discharge, LED, and other.
When SPDs are available, the aggregate lighting efficiency for a particular year can be found by
%
\begin{equation} \label{eq:agg_eta}
  \eta_{X,L,v,agg} = \sum \limits_{i} \theta_{i} \, \eta_{X,L,v,i} \; .
\end{equation}
%
****
ZM: 
Please verify that I edited the above equation correctly.
I think you had it wrong.---MKH
****

When SPDs are unavailable, the approximate method can be used as follows:
%
\begin{equation} \label{eq:agg_eta_K}
  \eta_{X,L,v,agg} \approx \sum \limits_{i} \theta_{i} \, \bar{\phi}_{L,i} \, \gammaratavg{}_{,i} 
...
\begin{enumerate}[label={(\alph*)}]
  ...
  \item light which is incident upon surfaces that do not need to be illuminated,
        ...
        **** This loss refers to the fact that objects such as ceilings, the tops of cabinets etc 
        ...
        Or at least they do not need to be illuminated with equal intensity! ---ZM ****
...
Several recent developments in the photometry and thermodynamics of radiation literatures 
(Petela's equation;
the photon effective temperature, $T_\lambda$; and
the spectral exergy-to-energy ratio, $\phi_{L,\lambda}$) 
have enabled
...
We developed an exact method for determining the exergetic efficiency of lamps
involving the 
spectral exergy-to-energy ratio~($\phi_{L,\lambda}$), 
a luminous weighting function~($f_\lambda$), and 
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item uses the correct spectral exergy-to-energy ratio~($\phi{L,\lambda}$), and
  
  \item allows choices for the spectral weighting function ($f_\lambda$),
    
\end{enumerate*}
%
all improvements over the conventional method.
...
we developed an approximate method,
involving surprisingly stable values of the exergy-to-energy ratio~($\phi_{L,\lambda}$) and 
the photopic-to-universal factor~($\gammarat{}$) across lighting technologies.  
Finally, we provided recommendations for societal exergy analysts, namely to
%
\begin{enumerate*}[label={(\alph*)}]
	
  \item use the exact method when a lamp's spectral power distribution is available, 
  
  \item use the universal luminosity weighting function~($U_\lambda$), and
  
  \item use the approximate method when luminous efficacy is known
...
\begin{enumerate*}[label={(\alph*)}]
	...
	\item investigating additional lighting technologies to assess the stability of 
	      the exergy-to-energy ratio~($\phi_L$) and the photopic-to-universal factor~($\gammarat{}$),
	
  \item applying the framework developed in this paper to understand the evolution
        of lighting technology relative to upper limits to
        luminous efficacy~($K$) and lighting efficiency~($\eta_{X,L,v}$),
...
fellowship grant RG.EVEA.112727.001.
