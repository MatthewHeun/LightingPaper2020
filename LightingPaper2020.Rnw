...
the lighting~($L$) and valuable~($v$) subscripts are implied and replaced by a weighting function specifier.
(E.g., the valuable energy efficiency of light for the visible luminous weighting function,
$\eta_{E,L,v,vis}$ is written as $\eta_{E,vis}$.)
...
Average conversion factors~($\gammaratavg{}$) by lamp technology are given in Table~\ref{tab:conversionfactors} in the appendix.
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item pushing toward the services stage of the energy conversion chain
        ...
        (caused by 
        excessive intensity, 
        lighting that falls on surfaces that do not require illumination, or
