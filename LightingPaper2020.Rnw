...
\ins{(Data for Figure~\ref{fig:LuminousEfficacy_Graph} were obtained from three sources: 
...
<<LuminousEfficacy_Graph, cache=use_cache, fig.align="center", fig.width = 5, fig.height = 4, fig.cap = "The luminous efficacy of selected lamp technologies from 1880 to 2020. Triangles indicate lamps used as examples in this paper." >>=
...
To our knowledge, 
the earliest reference for 220~lm/W and 400~lm/W is \citet{Summers1971} (p.~151),
...
for regulation of body temperature~\cite{Jones:2016aa} (p.~258).
...
such as exergy and entropy''~\citep{liu2009} (p.~1810).
