...
Average exergy-to-energy ratios~($\bar{\phi}_{L,pl}$) by lamp technology
are given in Table~\ref{tab:mean_phi_p2_by_lamp_type} in the appendix.
...
with a small standard deviation of $\sigma_{\gammarat{}} = \Sexpr{format(round(conversionX_sd, 3), nsmall = 3)}$.
Average conversion factors~($\gammaratavg{}$) by lamp technology
are given in Table~\ref{tab:conversionfactors} in the appendix.
...
# Creates a matrix with each element containing the number of decimal places that the xtable should use.
mdat_conv<-matrix(2,nrow=3,ncol=7); mdat_conv[2,]<-3;  mdat_conv[3,]<-0
