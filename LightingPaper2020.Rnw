...
****
ZM: 
Please fill the standard deviations above.---MKH
****
Average conversion factors~($\gammarat{}$) per lamp technology are given in Table~X.

The approximate method for determining the valuable exergetic efficiency of a lamp
is implemented by the following equation when the lamp technology is unknown:
%
\begin{equation} \label{eq:approximate}
  \eta_{X,L,v} \approx \bar{\phi}_{L} \, \gammaratavg{} \, \frac{K}{K_{max}} 
  = (0.958) (1.37) \frac{K}{K_{max}} 
  = 1.31 \frac{K}{K_{max}} \; .
\end{equation}
%
If the lamp technology is known, values from Table~X should be used, instead.
****
ZM:
Please complete Table~X---MKH
****
Results for the valuable exergetic efficiency 
of the four representative lamps used in this study
and each of three methods (conventional, exact, and approximate)
is given in Table~\ref{tab:threemethods}.
The ``Approximate method'' row was generated using
the factors in Equation~\ref{eq:approximate}.
