...
\ins{overestimation of exergetic efficiency by a factor of 3.}
In response, 
...
Ahead of a detailed treatment of the thermodynamics of light in Section~\ref{sec:framework}\ins{,}
...
<<Vis_Graph, cache=use_cache, fig.align="center", fig.width = 5, fig.height = 3, fig.cap="Normalized sensitivity and color perception of the human eye to EM radiation~\\cite{schubert2006}, according to the photopic luminous weighting function~($V_\\lambda$). This figure and Figure~\\ref{fig:wfs_and_lamps} below utilize the CIE 10 degree color matching function \\ins{(CMF)~\\cite{CIE2006}.We note that wavelengths of light are not inherently colored, but we have adopted this approach for demonstration purposes.} \\ins{The \\texttt{R} package \\texttt{ggspectra}~\\cite{Aphalo2015} was used to generate both figures.} 555~nm \\rev{is the wavelength of peak human eye visibility}{represents the peak of the combined sensitivities of two of the human eyes five light recpetors, the long-wavelength and medium-wavelength cones}, \\humaneyesensitivity{} is the range of human retina sensitivity, and 250~nm < $\\lambda$ < 900~nm is the range of data availability for lamp spectral power distributions (SPDs) in the Light Spectral Power Distribution Database (LSPDD)~\\citep{Roby, miguel2017, aube2013}.">>=
response_data %>% 
  ...
  scale_x_continuous(limits = c(250, 900), breaks = c(250, 380, 555, 780, 900)) + 
...
\ins{Thus, societal exergy analysis 
...
\rev{Thus}{Therefore}, the efficiency of energy and exergy conversion from electricity
to light by electric lamps~($\eta_E$ and $\eta_X$, respectively)
is the focus of this article.
Note that both energy services (e.g., illumination)~\ins{, the stage of the energy conversion chain
termed `application efficacy' in photometry \\cite{rea2001application},} 
...
Tsao and Waide, 
% \citeauthor{Tsao2010appetitelight}, 
who state 
...
To our knowledge, 
the earliest reference for 220~lm/W and 400~lm/W is \citet[p.~151]{Summers1971},
...
The value of 683~lm/W is \\rev{the maximum efficacy} {derived from the definition of the lumen. 
...
with electricity is converted into radiation with at 100~\% efficiency.}

Paoli and Cullen
% \citeauthor{Paoli2020} 
estimated the practical upper limit for the luminous efficacy of an LED lamp to be 284--350~lm/W,
when considering all \rev{stages}{sources} of energy loss
\ins{(driver efficiency, wall plug efficiency, optical efficiency, and spectral efficiency)}~\citep{Paoli2020}.
...
Figure~\ref{fig:LuminousEfficacy_Graph} shows that the 
lower efficacies from Summers (220 lm/W) and
Paoli and Cullen (284 lm/W)
% \citeauthor{Paoli2020} 
are within reach,
due to the rapid improvement of LED technology.
...
\end{align}
%
\ins{(Note that the overdot notation, as in $\dot{E}$, $\dot{Q}$, and $\dot{W}$,
indicates a steady state rate of energy or exergy flow.)}
...
with peak sensitivity at 555~nm \del{(green)}
...
emitting \del{exclusively green} light at $\lambda = 555 \text{ nm}$.
...
Because lamps in the LSPDD are not measured using an \rev{integration}{integrating} sphere,
...
tibble::tibble(lambda = seq(250, 900, by = 0.5)*10^(-9)) %>% 
  ...
  scale_x_continuous(limits = c(250, 900.01), breaks = c(250, 380, 555, 780, 900)) +
