...
I don't think we have a name for the bottom row.---MKH
****
...
so analysts must apportion per-lamp-technology efficiencies
by their proportion of lighting electricity consumption for each lighting technology~($\theta_i$).
Several types of data are required for a given period of interest, typically a year:

\begin{enumerate}[label={(\alph*)}]

  \item efficiency data for each lamp technology~($\eta_{X,L,v,i}$),
