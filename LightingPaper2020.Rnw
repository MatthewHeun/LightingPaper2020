...
\usepackage{textcomp}                       % For the single vertical apostrophe (')
...
overestimation of exergetic efficiency by a factor as large as 3.
In response, 
...
\ins{We note that wavelengths of light are not inherently colored, 
but we have adopted a color representation in Figures~\ref{fig:Vis_Graph} 
and~\ref{fig:wfs_and_lamps} for demonstration purposes.
The \texttt{R} package \texttt{ggspectra}~\cite{Aphalo2015} 
was used to generate both figures.
In the figures, 555~nm represents the peak of the combined sensitivities 
...
<<Vis_Graph, cache=use_cache, fig.align="center", fig.width = 5, fig.height = 3, fig.cap="Normalized sensitivity and color perception of the human eye to EM radiation~\\cite{schubert2006}, according to the photopic luminous weighting function~($V_\\lambda$). This figure and Figure~\\ref{fig:wfs_and_lamps} utilize the CIE 10 degree color matching function (CMF)~\\cite{CIE2006}. We note that wavelengths of light are not inherently colored, but we have adopted this approach for demonstration purposes. The \\texttt{R} package \\texttt{ggspectra}~\\cite{Aphalo2015} was used to generate both figures. 555~nm represents the peak of the combined sensitivities of two of the human eyes' five light receptors, the long-wavelength and medium-wavelength cones. The range of human eye sensitivity is \\humaneyesensitivity{}, and 250~nm < $\\lambda$ < 900~nm is the range of data availability for lamp spectral power distributions (SPDs) in the Light Spectral Power Distribution Database (LSPDD)~\\citep{Roby, miguel2017, aube2013}.">>=
response_data %>% 
...
Note that both energy services (e.g., illumination, the stage of the energy conversion chain
...
The value of 683~lm/W is derived from the definition of the lumen. 
...
when considering all sources of energy loss
...
``what is the maximum work we could obtain from a stream of the physical property?''
...
\end{equation}
%
See~\citet{benenson2006handbook}.)
...
emitting light at $\lambda = 555 \text{ nm}$.
...
250~nm < $\lambda$ < 900~nm, and}
\del{The} data \del{in the LSPDD }are provided 
in terms of \emph{relative intensity} ($\dot{E}_{EM,\lambda,rel}$) 
as opposed to \emph{absolute intensity} ($\dot{E}_{EM,\lambda}$).
Because lamps in the LSPDD are not measured using an integrating sphere,
...
That said, we suggest that the choice of weighting function in societal exergy analysis
