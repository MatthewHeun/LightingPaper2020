...
In response, 
we use recent results from the 
fields of photometry and radiation thermodynamics to
...
(b)~uses a spectral exergy-to-energy ratio, and 
...
for societal exergy analysts: use
...
<<Vis_Graph, cache=use_cache, fig.align="center", fig.width = 5, fig.height = 3, fig.cap="Normalized sensitivity and color perception of the human eye to EM radiation~\\cite{schubert2006}, according to the photopic luminous weighting function~($V_\\lambda$). 555~nm is the wavelength of peak human eye visibility, \\humaneyesensitivity{} is the range of human retina sensitivity, and 250~nm < $\\lambda$ < 900~nm is the range of data availability for lamp spectral power distributions (SPDs) in the Light Spectral Power Distribution Database (LSPDD)~\\citep{Roby, miguel2017, aube2013}.">>=
response_data %>% 
...
Thus, the efficiency of energy and exergy conversion from electricity
to light by electric lamps~($\eta_E$ and $\eta_X$, respectively)
...
electricity (in watts, W) as the energy input in the denominator
...
Practitioners who use luminous efficacy 
as a lamp efficiency metric include \citet{Nordhaus1996} 
and \citeauthor{Tsao2010appetitelight}, who state 
``[l]uminous efficacy represents the efficiency 
with which energy is used to produce visible light''~\citep[p.~265]{Tsao2010appetitelight}
...
but lighting efficiencies are given in 
...
The conventional method is used also to determine the efficiency of 
ammonia and steel production processes.
**** Can whoever made this comment provide a reference?---MKH ****
...
220~lm/W \cite{Summers1971, Ayres2003, USDepartmentofEnergyDoE2011},
400~lm/W \cite{Summers1971, Ayres2005, Tsao2010solidstate, Summers1971, Guevara2016}, and
683~lm/W \cite{Kondo2009, Serrenho2014}, based on the maximum efficacy of 
a ***** PB: I added Summers references to 220 and 400, as requested by PB.
But I don't think this is done.
We still need the specific reason why 220 and 400 were chosen.
I think ``broad-spectrum white light'' came from PB originally.
I don't know if ``broad-spectrum white light'' is the same as 
``acceptable white light.''---MKH ****, 
...
due to the rapid improvement in luminous efficacies for LED technology.
...
fields of photometry and radiation thermodynamics
(Petela's equation;
