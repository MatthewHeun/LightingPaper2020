...
\begin{enumerate}[label={(\alph*)}]
  ...
  \item it uses a non-unity spectral exergy-to-energy ratio, no longer assuming $\phi_{L,\lambda} = 1$, and 
...
**** URL has been reserved and 
will be activated by University of Leeds upon acceptance for publication ****:
