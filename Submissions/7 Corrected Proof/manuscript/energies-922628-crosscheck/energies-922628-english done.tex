...
\history{Received: 20 August 2020; Accepted: 12 October 2020; Published: date}
...
\Author{\hl{Matthew Kuperus Heun} $^{1,}$*\orcidA{}, 
        Zeke Marshall $^{2}$\orcidB{}, 
        Emmanuel Aramendia $^{2}$\orcidC{} and 
        \mbox{\hl{Paul E. Brockway} $^{2}$\orcidD{}}}
...
the exergy of light and the exergetic efficiency of lamps. The exact method 
...
is consumed for the purpose of making artificial light~\cite{waide2006}. With the breakthrough technology of light emitting diode (LED) lamps,
...
%
where 
$h$ is Planck's constant ($6.626 \times 10^{-34} \text{\hl{ J-s}}$), %MDPI: Please check the format of symbol.
...
of the \hl{CIE} 10 degree color matching function (CMF)~\cite{CIE2006}, %MDPI: Please modify CIE.
supplied by the \hl{\texttt{R}} package\hl{ \texttt{ggspectra}}~\cite{Aphalo2015}, %MDPI: Is the command necessary？
\mbox{for demonstration} purposes
in \hl{Figures}~\ref{fig:Vis_Graph} 
...
The need leads to the \hl{\emph{aim}} of this article:  %MDPI: Is the italic format necessary?
to bring clarity to 
%
...
The novel \hl{\emph{contributions}} of this article include:
%
\begin{enumerate*}[label={(\alph*)}]
...
The \hl{\emph{structure}} of this article is as follows:
...
presenting a discussion in Section~\ref{sec:discussion} and \ref{sec:conclusions} summarizes and {suggest future work}\DIFaddend .
...
\hl{(} %MDPI: Please check if it can be deleted.
...
Thus, the \hl{\emph{valuable}} energy efficiency ($\eta_{E,v}$) is 
...
radiant power by \hl{$\Phi$}, not $\dot{E}$. %MDPI: Please confirm whether it need  a italic format.
...
so we reserve \hl{$\Phi$} for lumen output of a lamp. 
...
Thus, the exergy of the first three streams in Figure~\ref{fig:lamp} is given \hl{by} %MDPI: Please check Equation 20.
\begin{align}
...
See~\citet{benenson2006handbook}\hl{)}. Visually, $\dot{E}_{EM}$ is the area under the $\dot{E}_{EM,\lambda}$ vs.\ $\lambda$ curve.%mdpi:Please fill in the brackets
...
%
are independent of human presence.
But what about \mbox{human \hl{\emph{perception}?}} %MDPI: Is the italic necessary?
...
For the first option, the degenerate (unweighted, \hl{uw}) %MDPI: Please check it and uniform it with Equation 38.
...
deem a lamp's EM radiation to be useful if and only if it \hl{\emph{could}}
...
was defined by the Commission Internationale de l'Eclairage \hl{(CIE)} in 1931%MDPI: Is this CIE  the same one with the former CIE in page 2?
and represents the sensitivity of M and L cones
at daytime light levels \cite{CVRL2008}.
So for \hl{Option 3},%Please uniform the big format.
...
in terms of \hl{\emph{relative intensity}} ($\dot{E}_{EM,\lambda,rel}$) 
as opposed to \hl{\emph{absolute intensity}} ($\dot{E}_{EM,\lambda}$).
...
All graphs were constructed and calculations were performed \hl{using} \texttt{R}~\cite{R-software} \hl{(Version, Company/Manufacturer, City, State abbrev if USA or Canada, Country)}.%MDPI: Please confirm whether R55 is a  software? If it is, please add the information.
...
\caption{Valuable energy efficiencies
                     ...
                     \mbox{given by} Equation~(\ref{eq:eta_E_v_light}). Note that for the incandescent (INC) and halogen (HAL) lamps, 
...
the exergy-to-energy ratio (\hl{$\phi_{L,\lambda}$)} is always close to~1 for EM wavelengths relevant to light.
...
of the luminous power (\hl{$\Phi_{pl}$}, Equation~(\ref{eq:lumen})), expressed in lm, and
...
\begin{enumerate}[leftmargin=8mm,labelsep=3mm] 
  ...
  \item[(2)] $\phi_{L,\lambda} = 1$ for all wavelengths, 
...
assumes the \hl{\emph{exergetic}} efficiency of a lamp
to be its \hl{\emph{energy}} efficiency.
...
should be appropriate for each lamp's application. Our second recommendation is for use of
...
%..............................
\paragraph{\emph{\hl{Implementing} the approximate method}}%MDPI: Is the italic necessary?
%..............................
...
Pedro J. Aphalo for
  his open-source software \hl{suite} \texttt{r4phototbiology}~\cite{Aphalo2015},%MDPI: Please confirm the format of the word ''r4phototbiology''.
...
\newblock \emph{Data Associated with ``The Energy and Exergy of Light with
  Application to Societal Exergy Analysis''};  \hl{Publisher, City, 2020.} %Please add the publisher and location.
  \newblock
